...
public abstract class HslUdpBaseProtocolDriver<T> : IProtocolDriver where T : DeviceUdpNet
{
    protected T? _conn;//协议连接对象
    private readonly string _protocolName;//协议名称
    private static readonly Dictionary<DataType, Func<HslUdpBaseProtocolDriver<T>, PointEntity, CancellationToken, Task<(bool IsSuccess, object? Content, string Message)>>> _readFuncs =
      new()
      {
    ...
    private static readonly Dictionary<DataType, Func<HslUdpBaseProtocolDriver<T>, WritePoint, CancellationToken, Task<bool>>> _writeFuncs =
    new()
    {
    ...
    protected virtual void SetStationNoIfNeed(WorkstationEntity protocol, PointEntity point)
    {
    ...
    protected virtual void SetDataFormatNoIfNeed(WorkstationEntity protocol, PointEntity point)
    {
        switch (_conn)
        {
            case ModbusTcpNet modbus:
