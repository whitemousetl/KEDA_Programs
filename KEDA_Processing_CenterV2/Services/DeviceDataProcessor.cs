...
public class DeviceDataProcessor : IDeviceDataProcessor
{
    ...
    public ConcurrentDictionary<string, string> Process(ProtocolResult protocolResult, ProtocolDto protocol, CancellationToken token)
    {
        ...
        foreach (var deviceResult in protocolResult.DeviceResults)
        {
            ...
            var device = protocol.Equipments.FirstOrDefault(d => d.Id == deviceResult.EquipmentId); //从协议中找到对应设备
    ...
    private void ProcessDeviceResult(DeviceResult deviceResult, EquipmentDto device, ConcurrentBag<ParameterDto> virtualPoints, ConcurrentDictionary<string, object?> forwardDeviceResult)
    {
        foreach (var pointResult in deviceResult.PointResults)
        {
            if (!IsValidPointResult(pointResult)) continue; // 点结果或点结果标签为空， 跳过当前设备结果
            var point = device.Parameters.FirstOrDefault(p => p.Label == pointResult.Label); //从设备中找到与设备结果标签一样的点，目的找到配置中该点的转换条件或虚拟点
    ...
    private void SerializeDeviceData(DeviceResult deviceResult, ConcurrentDictionary<string, object?> forwardDeviceResult, ConcurrentBag<ParameterDto> virtualPoints, ConcurrentDictionary<string, string> dataDevId)
    {
