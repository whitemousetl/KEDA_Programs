...
public abstract class TcpBaseProtocolDriver<T> : BaseProtocolDriver<T> where T : DeviceTcpNet
{
    protected override async Task OnConnectionInitializedAsync(CancellationToken token)
    {
        if (_conn != null)
        {
            var connRes = await _conn.ConnectServerAsync();
            if (!connRes.IsSuccess)
                throw new ProtocolWhenConnFailedException($"{_protocolName}协议连接PLC时异常:  {connRes.Message}",
