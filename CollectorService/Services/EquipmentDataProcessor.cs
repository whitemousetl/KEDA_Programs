...
public class EquipmentDataProcessor : IEquipmentDataProcessor
{
    ...
    public ConcurrentDictionary<string, string> Process(ProtocolResult protocolResult, List<EquipmentDto> equipments, CancellationToken token)
    {
        ...
        foreach (var equipmentResult in protocolResult.EquipmentResults)
        {
            ...
            forwardEquipmentResult["DeviceId"] = equipmentResult.EquipmentId;
            forwardEquipmentResult["timestamp"] = timestamp;

            var equipment = equipments.FirstOrDefault(d => d.Id == equipmentResult.EquipmentId); //从协议中找到对应设备
