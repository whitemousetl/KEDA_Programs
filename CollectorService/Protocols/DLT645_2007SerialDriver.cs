...
public class DLT645_2007SerialDriver : IProtocolDriver
{
    private DLT645? _conn;
    private string _protocolName = "DLT645_2007Serial";
    ...
    public async Task<PointCollectTask?> ReadAsync(Protocol protocol, Device device, Point point, CancellationToken token)
    {
        try
        {
            if (_conn == null)
            {
                ...
                var baudRate = int.Parse(protocol.BaudRate);
                var dataBits = int.Parse(protocol.DataBits);
                var stopBits = Enum.Parse<StopBits>(protocol.StopBits);
                var parity = Enum.Parse<Parity>(protocol.Parity);

                _conn.SerialPortInni(protocol.PortName, baudRate, dataBits, stopBits, parity);
                if (!_conn.IsOpen())
                {
                    var connRes = _conn.Open();
                    if (!connRes.IsSuccess)
                        throw new ProtocolFailedException($"{_protocolName}协议打开串口失败: {connRes.Message}", new Exception(connRes.Message));
